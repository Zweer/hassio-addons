...
if [ -f "$CONFIG_PATH" ]; then
    ...
    TELEGRAM_IDS=$(python3 -c "import json; ids=json.load(open('$CONFIG_PATH')).get('telegram_allowed_user_ids', ''); print(json.dumps([int(x.strip()) for x in ids.split(',') if x.strip()] if ids else []))")
    DISCORD_TOKEN=$(python3 -c "import json; print(json.load(open('$CONFIG_PATH')).get('discord_bot_token', ''))")
    DISCORD_IDS=$(python3 -c "import json; ids=json.load(open('$CONFIG_PATH')).get('discord_allowed_user_ids', ''); print(json.dumps([x.strip() for x in ids.split(',') if x.strip()] if ids else []))")
